...
const styles = tv({
	...
	compoundVariants: [
		{ position: 'start', direction: 'horizontal', class: { line: 'top-8 h-px w-full bg-utility-neutral-200/50' } },
		{ position: 'end', direction: 'horizontal', class: { line: 'bottom-8 h-px w-full bg-utility-neutral-200/50' } },
		{ position: 'start', direction: 'vertical', class: { line: 'left-8 h-full w-px bg-utility-neutral-200/50' } },
		{ position: 'end', direction: 'vertical', class: { line: 'right-8 h-full w-px bg-utility-neutral-200/50' } },
	],
});

/**
 * Properties for the {@link MenuItemIcon} component.
 * Extends primitive `div` element props while omitting polymorph controls and child nodes.
 */
export interface MenuItemIconProps extends Omit<BoxProps<'div'>, 'as' | 'asChild' | 'children'> {
	/**
 ...
 *
 * @param props - Component configuration options defined by {@link MenuItemIconProps}.
 * @returns The rendered icon badge node with decorative structural accents.
 */
export const MenuItemIcon = (props: MenuItemIconProps): JSX.Element => {
