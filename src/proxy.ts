 ...
 * @returns The extracted supported locale of undefined if missing or unsupported.
 */
export const extractLocaleFromPath = (pathname: string): Language | undefined => {
	const targetPathnameSegment = pathname.split('/')[1] as Language;
	return SUPPORTED_LOCALES.includes(targetPathnameSegment) ? targetPathnameSegment : undefined;
};
 ...
 */
export const readLocaleFromCookie = (req: NextRequest): Language | undefined => {
	const cookieValue = req.cookies.get(LOCALE_COOKIE_CONFIG.name)?.value as Language | undefined;
 ...
 */
export const negotiateHeaderLocale = (req: NextRequest): string => {
 ...
 */
export const resolvePreferredLocale = (req: NextRequest): string => {
 ...
 */
export const createSyncCookieResponse = (locale: Language): NextResponse => {
	const response = NextResponse.next();

	response.cookies.set(LOCALE_COOKIE_CONFIG.name, locale, {
		path: '/',
 ...
 */
export const createRedirectResponse = (req: NextRequest, targetLocale: string): NextResponse => {
	const targetURL = new URL(`/${targetLocale}${req.nextUrl.pathname}`, req.url);
	targetURL.search = req.nextUrl.search;

	const response = NextResponse.redirect(targetURL);

	response.cookies.set(LOCALE_COOKIE_CONFIG.name, targetLocale, {
		path: '/',
		maxAge: LOCALE_COOKIE_CONFIG.maxAge,
	});
 ...
 */
export const handleExistingLocalePath = (req: NextRequest, currentPathLocale: Language): NextResponse | undefined => {
	const localeCookie = readLocaleFromCookie(req);

	const isCookieOutdated = currentPathLocale !== localeCookie;
	if (isCookieOutdated) return createSyncCookieResponse(currentPathLocale);
 ...
 */
export const handleMissingLocalePath = (req: NextRequest): NextResponse => {
